...
| `crates/campus-synjones/` | 慧新E校协议：lyCas 桥换 token（单实例缓存）、一卡通卡信息与流水（`berserker-app` + `berserker-search`）、电费三级级联（`charge` 系）、`synAccessSource=app` 双份携带 | [[modules/campus-synjones|慧新E校协议核心]] |
