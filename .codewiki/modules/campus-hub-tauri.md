...
`tauri-app/src-tauri`（crate 名 `campus-hub`）是协议核心与前端之间的 IPC 接线层：命令面、AppState、DPAPI 持久化。协议逻辑零实现——「协议核心全部在 campus-auth crate，本 crate 只做 IPC 接线与本地持久化」（`src/lib.rs:2`）。登录/账号命令定义在 `src/commands/auth.rs`，头像/资料命令定义在 `src/commands/profile.rs`，门户数据命令定义在 `src/commands/portal.rs`，一卡通命令（M4.5）定义在 `src/commands/ecard.rs`，全部注册于 `lib.rs:19-112`。

## 命令面（下表覆盖 auth / profile / portal / timetable；慧新E校与电费的 43 条（synjones 3 / ecard 22 = 读 7 + 写 15 / electricity 12 / electricity_history 6）见 `modules/campus-synjones.md`、`modules/ecard-panel.md` 及下方 M4 小节；**全量 92 条**的命令面总账在根 `_architecture.md`「命令面与模块地图」）
...
| `save_skipped_dates` | `dates: Vec<NaiveDate>` | `TimetableView`（跳过日期整体替换但**自动并入 `holiday_names` 记录的法定假日**——用户编辑只管手动停课日；升序+去重归一。批 2 2026-09-19 新增、2026-09-20 修订，契约 §8.2） | `timetable.rs` |
...
- **`import_timetable` 链路**（`timetable.rs:75-150`）：门户学期信息（会话内已缓存）推导 `xnm`/`xqm`（冻结契约 §1.2 口径：`xnm`=`start_date` 前 4 位、`semester` `"1"→3/"2"→12`，**不用 `grade`**）→ `fetch_timetable_json`（901→TGT 静默重进在 campus-auth 内部；失败 Display 中文直接透出，`JwglNotLogin` =「教务会话已失效，请重新登录」）→ `parse_kb_response(json, DEFAULT_TABLE_ID)` → [[modules/campus-schedule|课表核心]] `diff_courses` 合并旧库 → 落库 → `ImportResult{added, changed, removed, total, changes}`（total = 合并后课程总数，含停开保留记录）。学期信息同时初始化/更新 `semester_start_date`（`"YYYYMMDD"`→`NaiveDate`）与 `semester_total_weeks`，**单字段解析失败保留旧值**（不因坏数据丢课表）。 2026-09-20 抽取内核 `run_timetable_import(client, tgt, portal)`（命令与每日自动任务共用），落库前做**教务为准清理**：① `config.swap_days` 中「置换日所在教学周该星期已有导入课」（`weekday_covered`）的置换删除——教务 kbList 以「新增同教学班条目」表达调休，同一事实的公告置换冗余（见 [[learnings/zhengfang-tiaoxiu-swap-entries|教务调休条目形态与公告置换冗余]]）；② `redundant_extra_override_ids` 判定的冗余 extra override 删除（旧逐课补课残留，防同格重复）；③ 写 `config.last_auto_import`（手动导入也算当天已同步）。
...
## 每日自动同步与节假日刷新（2026-09-20 节假日轮）

- **`auto_sync_tick(app)`**（`commands/timetable.rs`，`pub(crate)`）：后台任务单次 tick——① 法定节假日刷新（闸 `config.last_holiday_fetch`）；② 教务课表导入（闸 `config.last_auto_import`，手动导入也写闸）。lib.rs `.setup()` spawn 循环：启动即检查一次，此后 `tokio::time::sleep(3600s)` 每小时检查（覆盖长开跨天）。护栏：无会话**静默跳过**（绝不在后台拉起登录链路顶掉用户会话）、失败只 `log::warn`（下小时重试）、不弹窗不阻塞启动；日志不含凭据。
- **`refresh_holidays_inner(today)`**（`fetch_holidays` 命令与后台共用）：timor.tech `holiday/year/{当年}`+`{次年}`（第一学期 9 月开学跨年；次年数据年底才发布，失败宽容）→ `campus_schedule::parse_timor_year` 解析（只取放假日）→ `merge_holidays` 合并（先剔旧自动假日再并入；`holiday_names` 整体替换）→ 落库写闸。需浏览器 UA（`TIMOR_UA`，否则 Cloudflare 拦截）。调休补班日（`holiday=false`）忽略——补班日上哪天的课以**教务调休条目**（每日自动导入）为准，公告置换（`apply_swap_day`）只作教务未表达时的兜底。
- **`apply_swap_day` 公告辅助化校验**：采纳前检查置换日所在教学周该星期是否已有导入课（`weekday_covered`）——教务已表达则 err「教务课表已包含 X（周Y）的调休安排，以教务为准，无需采纳该置换」（前端经 noticeMsg 展示）。`week < 1`（置换日不在学期）不拦。
- 架构口径：**教务系统为准**（每天自动导入一次）+ **公告解析辅助**（教务未覆盖时兜底）+ **法定节假日跳过**（timor 假日并入 `skipped_dates`：网格「休+节日名」列、ICS 剔除、今日页 skipped 全部复用既有消费方，前端零改动——`holidayNames` 字段与渲染上轮已预留）。与电费补采、节假日刷新、课表导入之间无进程内互斥（沿用契约 §2.2 调用方串行假设，极端并发后写覆盖、数据下次刷新自愈，代码注释登记）。
