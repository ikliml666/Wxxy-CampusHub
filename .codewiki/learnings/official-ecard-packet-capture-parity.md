...
## 七、查询密码 60005 的真相（2026-09-20 晚间二次逆转：本节旧结论是错的）

> ⚠️ **本节原标题「穷举取证：应用侧无解的证据链」与结论「该校查询密码实际值并非证件
> 后六位」均已被推翻**，完整协议解释见
> [[learnings/ecard-keyboard-pseudochar-protocol|安全键盘伪字符映射协议]]。

- 当时穷举（Number/Standard uuid × app/h5 来源 × 卡号）全 60005——**方法错了**：把
  证件后六位明文直接拼进 pwd 提交。协议上 pwd 明文必须是键盘批次的**伪字符**（图片画
  数字、数组值是伪字符，服务端按 uuid 批次映射还原），纯数字明文 60005 是必然结果，
  与密码真值无关。
- 「官方 Number 键盘根本输不出固定的 6 位数字密码」同样不成立——官方键面是**图片
  数字九宫格**（`numberKeyboardImage` 每键一图画数字），用户完全输得出；我们当时只看
  了 `numberKeyboard` 伪字符数组。
- **最终实锤**：应用内 Number 键盘 + positions 链（按位置取伪字符拼 pwd）提交证件后六位
  **校验通过（retcode=0）**——用户始终是对的，密码就是证件后六位。
- 仍然成立的部分：`account` 必须是卡号（学工号 4000 越权）；「每次尝试都被服务端计
  数」的提醒合理（别把校验接口当试探工具）。
- 同日同源教训：官方「查看卡号」checkPwd 只是显示闸门，完整卡号随 `getCampusCards`
  的 `bankacc` 早已下发——官方通过后底部弹窗（前 4 位 + 按钮）才显全号，应用已同款
  复刻。
