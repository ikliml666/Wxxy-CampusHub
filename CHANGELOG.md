...
- **验证**：`cargo test --workspace` **322 passed / 0 failed / 13 ignored**（本轮新增单测 34：crate 侧 18 = `turnover.rs` 13 + `charge.rs` 4 + `ecard.rs` 1，tauri 侧 16 = `infra/electricity_history.rs` 10 + `commands/electricity_history.rs` 4 + `commands/electricity.rs` 2）；`tsc -p tsconfig.json` 前端类型检查零错（`SavedRoom.bound` 声明为可选，不破坏既有组件）；**真机点验与启动补采实测属批 4**，本轮未跑
